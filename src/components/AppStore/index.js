...
class AppStore extends Component {
  ...
  getTabIdItems = searchInputResult => {
    const {activeId} = this.state
    const FilteredItems = searchInputResult.filter(
      eachItemId => eachItemId.category === activeId,
    )
  ...
  render() {
    ...
    const searchInputResult = this.onSearchFilter()
    const GetTabResults = this.getTabIdItems(searchInputResult)

    return (
      <div className="container1">
        <h1 className="heading-app">App Store</h1>
        <div className="cont-to-search-input-icon">
          <input
            type="search"
            placeholder="Search"
        ...
        <ul className="ul-style-1">
          {tabsList.map(eachItem => (
            <TabItems
              tabsList={eachItem}
              key={eachItem.tabId}
              onClickingTabs={this.onClickingTabs}
              isTab={eachItem.tabId === activeId}
            />
          ))}
        </ul>
        <ul className="ul-style-2">
          {GetTabResults.map(eachApp => (
            <AppItems appsList={eachApp} key={eachApp.appId} />
          ))}
        </ul>
      </div>
    )
